...
class Migration001(Migration):
    ...
    def __init__(self):
        super().__init__(
            version=1, description="Add new article fields - `slug` and `priority`"
        )
    ...
    async def upgrade(self, db: AsyncIOMotorDatabase):
        ...
        no_slug_articles = (
            await db["articles"].find({"slug": {"$exists": False}}).to_list(length=None)
        )
        ...
        for t in titles:
            ...
            if title is not None:
                ...
                await db["articles"].update_one(
                    {"_id": ObjectId(id)}, {"$set": {"slug": slug, "priority": 0.8}}
                )

                await db["users"].update_one(
                    {"articles": {"$elemMatch": {"id": ObjectId(id)}}},
                    {"$set": {"articles.$.priority": 0.8, "articles.$.slug": slug}},
                )

    async def downgrade(self, db: AsyncIOMotorDatabase):
        await db["articles"].update_many({}, {"$unset": {"slug": 1, "priority": 1}})
